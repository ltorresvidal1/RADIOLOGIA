...
<div class="ms-auto">
    <a href="{{route('usuarios.create')}}" class="btn btn-primary"><i class="fa fa-plus-circle fa-fw me-1"></i> Crear Usuarios</a>
</div>
@endsection
            ...
            <td>
                ...
                <div class="dropdown text-center">
                    ...
                    <div class="dropdown-menu">
                        <form id="restablecer-{{$usuario->id}}" action="{{route('usuarios.restablecer',$usuario)}}" method="POST">
                            @csrf
                            @method('PUT')
                        </form>
                        <a  class="dropdown-item"  onclick="RestablecerClave('{{ $usuario->id }}')"><i class="fas fa-lg fa-fw me-2 fa-key"></i>Restablecer</a>
                        <a href="{{ route('usuarios.edit', $usuario->id) }}" class="dropdown-item"><i class="far fa-edit fa-fw fa-lg"></i> Editar</a>
                        <form id="delete-{{$usuario->id}}" action="{{route('usuarios.destroy',$usuario)}}" method="POST">
                            @csrf
                            @method('DELETE')
                        </form>
