...
class DatatableController extends Controller
{
    ...
    public function estudiosclientes(string $institucion,string $fechainicial,string $fechafinal){
      ...
      ->selectRaw("study.pk as study_pk,patient.pk ,patient_id.pat_id,concat( SUBSTRING(study.study_date, 7, 2) ,'/',SUBSTRING(study.study_date, 5, 2) ,'/',SUBSTRING(study.study_date, 0, 5)) as fecha,concat(person_name.family_name ,' ',person_name.given_name ,' ' ,person_name.middle_name,' ' , person_name.name_prefix) as nombre")
      ->distinct()->get();
     ...
     return datatables()->of($estudios)->addColumn('action',function ($estudios){
            ...
            $acciones.='<a title="Imprimir" class="w-30px h-30px bg-gradient-primary rounded-circle d-flex align-items-center justify-content-center"><div class=""><i class="fa fa-print text-white"></i> </div></a>';
